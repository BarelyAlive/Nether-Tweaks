...
public class TileEntityNetherrackFurnace extends TileNTMBase {
    ...
    public void update() {
        ...
        if (!worldObj.isRemote)
        {
            ...
            if (!isWorking() && canSmelt())
            {
            	if(workTime == 0)
            		workTime++;
            	worldObj.notifyBlockUpdate(pos, NTMBlocks.blockNetherrackFurnace_lit.getDefaultState(), NTMBlocks.blockNetherrackFurnace_lit.getDefaultState(), 3);
                if (isWorking())
                {
                    flag1 = true;
            ...
            {
                flag1 = true;
                NetherrackFurnace.setState(isWorking(), worldObj, pos);
            }
    } 
    ...
    public void smeltItem()
    {
        if (canSmelt())
        {
            ...
            {
                this.inv[1].stackSize += itemstack.stackSize; // Forge BugFix: Results may have multiple items
            }
