...
public class BucketWood extends Item implements IFluidHandler, IVariantProvider{
	
	/** field for checking if the bucket has been filled. */
    private final Block containedBlock;
	
	public BucketWood(Block containedBlockIn, String unlocalizedName){
        ...
        else
        {
            ...
            else if (flag)
            {
                ...
                else
                {
                    ...
                    if (material == Material.WATER && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0)
                    {
                        ...
                        return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, this.fillBucket(itemstack, playerIn, BucketLoader.bucketWoodWater));
                    }
                    else if (material == Material.WATER && iblockstate.equals(BucketLoader.blockDemonWater) && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0)
                    {
                        ...
                        worldIn.setBlockState(blockpos, Blocks.AIR.getDefaultState(), 11);
                        playerIn.addStat(StatList.getObjectUseStats(this));
                        return new ActionResult<ItemStack>(EnumActionResult.SUCCESS, this.fillBucket(itemstack, playerIn, BucketLoader.bucketWoodDemonWater));
                    }
                    else
                    {
                        return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
            ...
            else
            {
                ...
                else if (this.tryPlaceContainedLiquid(playerIn, worldIn, blockpos1))
                {
                    ...
                    playerIn.addStat(StatList.getObjectUseStats(this));
                    return !playerIn.capabilities.isCreativeMode ? new ActionResult(EnumActionResult.SUCCESS, new ItemStack(BucketLoader.bucketWood)) : new ActionResult(EnumActionResult.SUCCESS, itemstack);
                }
                else
                {
                    return new ActionResult<ItemStack>(EnumActionResult.FAIL, itemstack);
    ...
    public boolean tryPlaceContainedLiquid(@Nullable EntityPlayer player, World worldIn, BlockPos posIn)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if (!worldIn.isRemote && (flag || flag1) && !material.isLiquid())
                {
    ...
    public net.minecraftforge.common.capabilities.ICapabilityProvider initCapabilities(ItemStack stack, @Nullable net.minecraft.nbt.NBTTagCompound nbt) {
        if (this.getClass() == BucketWood.class)
        {
