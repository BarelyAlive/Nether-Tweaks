...
public class TeleporterNTM implements net.minecraftforge.common.util.ITeleporter
{
    ...
    public TeleporterNTM(WorldServer worldIn)
    {
        this.world = worldIn;
    ...
    public void placeInPortal(Entity entityIn, float rotationYaw)
    {
        ...
        else
        {
            ...
            {
            	if(!hasCoodData(((EntityPlayerMP) entityIn)))
            	{
            		((EntityPlayerMP) entityIn).bedLocation = new BlockPos(i, j, k);
            		((EntityPlayerMP) entityIn).setSpawnPoint(new BlockPos(i, j, k), true);
            		((EntityPlayerMP) entityIn).getEntityData().setInteger(WorldHandler.coodX, i);
            		((EntityPlayerMP) entityIn).getEntityData().setInteger(WorldHandler.coodY, j);
            		((EntityPlayerMP) entityIn).getEntityData().setInteger(WorldHandler.coodZ, k);
            	}
            }
        }
    }
    ...
    public void placeEntity(World world, Entity entity, float yaw)
    {
        ...
        {
        	System.out.println("Entity is not Player");
            placeInExistingPortal(entity, yaw);
        }
    }
    
    private boolean hasCoodData(EntityPlayer player)
    {
    	return player.getEntityData().hasKey(WorldHandler.coodX) && player.getEntityData().hasKey(WorldHandler.coodY) && player.getEntityData().hasKey(WorldHandler.coodZ);
    }
}