...
public class TeleporterNTM implements ITeleporter{

	protected final World world;
    /** A private Random() function in Teleporter */
    protected final Random random;
    /** Stores successful portal placement locations for rapid lookup. */
    protected final Long2ObjectMap<TeleporterNTM.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap<TeleporterNTM.PortalPosition>(4096);

    public TeleporterNTM(World worldIn)
    {
        this.world = worldIn;
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw)
    {
        ...
        {
            TeleporterNTM.PortalPosition teleporter$portalposition = (TeleporterNTM.PortalPosition)this.destinationCoordinateCache.get(l);
            d0 = 0.0D;
        ...
        if (d0 >= 0.0D)
        {
            if (flag)
            {
                this.destinationCoordinateCache.put(l, new TeleporterNTM.PortalPosition(blockpos, this.world.getTotalWorldTime()));
            }
            ...
            System.out.println((double)(blockpattern$patternhelper.getFrontTopLeft().getY() + 1));
            //System.out.println(entityIn.getLastPortalVec().y);
            System.out.println((double)blockpattern$patternhelper.getHeight());
            double d6;
            if (entityIn.getLastPortalVec() != null)
            {
            	d6 = (double)(blockpattern$patternhelper.getFrontTopLeft().getY() + 1) - entityIn.getLastPortalVec().y * (double)blockpattern$patternhelper.getHeight();
            }
            else
            {
            	d6 = (double)(blockpattern$patternhelper.getFrontTopLeft().getY() + 1) - entityIn.getPosition().getY() * (double)blockpattern$patternhelper.getHeight();
            }
            ...
            if (blockpattern$patternhelper.getForwards().getAxis() == EnumFacing.Axis.X && entityIn.getLastPortalVec() != null)
            {
            ...
            {
                d5 = d2 + (1.0D - entityIn.getPosition().getX()) * (double)blockpattern$patternhelper.getWidth() * (double)blockpattern$patternhelper.getForwards().rotateY().getAxisDirection().getOffset();
            }
    ...
    public void removeStalePortalLocations(long worldTime)
    {
        if (worldTime % 100L == 0L)
        {
            ...
            ObjectIterator<TeleporterNTM.PortalPosition> objectiterator = this.destinationCoordinateCache.values().iterator();

            while (objectiterator.hasNext())
            {
                TeleporterNTM.PortalPosition teleporter$portalposition = (TeleporterNTM.PortalPosition)objectiterator.next();
